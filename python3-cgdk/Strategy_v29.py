...
class Group:
    ...
    @staticmethod
    def next():
        group_id = Group.squadrons[-1] + 1
        Group.squadrons.append(group_id)
        return group_id
...
class MyStrategy:
    ...
    def move(self, me: Player, world: World, game: Game, move: Move):
        ...
        #if (self.units_have_stopped(self.me, group=Group.AERIAL) and 
        #        self.units_have_stopped(self.me, group=Group.GROUND)):
            for group in Group.squadrons:
                if self.count_free_facilities() > 0:
                    self.potential_flow(group)
                else:
                    self.attack_nearest_cluster_of_enemy(group)
        if self.execute_move_queue(move):
            return

        if self.can_nuke() and self.world.tick_index % 100 == 0:
            self.fire_nukes()
    ...
    def execute_move_queue(self, m):
        ...
        for key in vars(temp_m).keys():
            ...
            setattr(m, key, value)
        # print(self.world.tick_index, m.action)
        return True
    ...
    def occupy_nearest_facility(self):
        for group in Group.squadrons:
            ...
            # normalize
            r = np.hypot(dx, dy)
            dx = dx * min(r, ((60*max_speed)/r))
            dy = dy * min(r, ((60*max_speed)/r))
    ...
    def assign_new_groups(self):
        for id in self.facilities_owned_now:
            ...
            y_range = [facility.top, facility.top + 16]
            if facility.owner_player_id != self.me.id:
                continue
            new_units_cnt = 0
            ...
            self.m_queue.append(self.new_move(action=1,
                                              x_range=x_range,
                                              y_range=y_range))
            self.m_queue.append(self.new_move(action=4, group=group_id))
